// frontend/src/pages/DocxToPdf.jsx
...
function DocxToPdf() {
  ...
  const handleConversion = async (files) => {
    if (files.length === 0) {
      setResponseMessage('Please select a DOCX file.');
      return;
    }
    ...
    try {
      const response = await fetch(`${BACKEND_URL}/api/convert/any_to_pdf`, { // Use the generic endpoint
        method: 'POST',
        body: formData,
      });
      ...
      const result = await handleApiResponse(
        response, 
        outputFilename, 
        'application/pdf' // Expected media type
      );
  ...
  return (
    <div className="text-center py-10">
      <h1 className="text-4xl font-bold text-gray-800 mb-6">Convert Word (DOCX) To PDF</h1>
      <p className="text-lg text-gray-600 mb-8">
        Convert your Word document into a PDF document. Requires LibreOffice on the server.
      </p>
      ...
      <FileUploader onFilesSelected={handleConversion} multiple={false} accept=".docx,.doc" disabled={loading} />
